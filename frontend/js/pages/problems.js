...
async function loadProblems(retriesLeft = 2) {
  ...
  try {
    ...
    catSel.innerHTML = DOMPurify.sanitize('<option value="">All Categories</option>' +
    ...
    if (retriesLeft > 0) {
      tbody.innerHTML = DOMPurify.sanitize('<tr><td colspan="5" style="padding:24px;color:var(--muted)">⏳ Waking up server… this can take up to a minute on first load.</td></tr>');
      await new Promise(r => setTimeout(r, 4000));
      return loadProblems(retriesLeft - 1);
    }
    tbody.innerHTML = DOMPurify.sanitize('<tr><td colspan="5" style="padding:24px;color:var(--warn)">⚠ Cannot reach API. Please refresh the page.</td></tr>';
  }
}
...
function filterProblems() {
  ...
  if (!filtered.length) {
    tbody.innerHTML = DOMPurify.sanitize('<tr><td colspan="5" style="padding:24px;color:var(--muted)">No problems match your filters.</td></tr>');
    return;
  }

  const isAdmin = document.getElementById('adminBadge')?.style.display !== 'none';
  tbody.innerHTML = DOMPurify.sanitize(filtered.map((p, i) => {
    ...
    return `<tr onclick="openProblem(${JSON.stringify(p).replace(/"/g,'&quot;')})"
      ...
      <td style="padding:12px 16px;text-align:center"><span class="badge badge-${escapeHTML(p.difficulty)}">${escapeHTML(p.difficulty)}</span></td>
