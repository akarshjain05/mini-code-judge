...
async function loadDashboard() {
  ...
  if (!token) { el.innerHTML = DOMPurify.sanitize('<p style="color:var(--muted);font-size:13px">Login to see your recent submissions.</p>'); return; }
  try {
    ...
    if (res.status === 401) {
      ...
      el.innerHTML = DOMPurify.sanitize('<p style="color:var(--muted);font-size:13px">Session expired. Please <a href="#" onclick="openAuthModal()" style="color:var(--accent)">log in again</a>.</p>');
      return;
    }
    ...
    if (!subs.length) { el.innerHTML = DOMPurify.sanitize('<p style="color:var(--muted);font-size:13px">No submissions yet. <a href="#" onclick="goTo(\'problems\')" style="color:var(--accent)">Try a problem →</a></p>'); return; }
    el.innerHTML = DOMPurify.sanitize(subs.map(s => {
      ...
      <div onclick="openSubmissionViewer(${s.id})" style="display:flex;align-items:center;gap:12px;padding:16px 24px;border-bottom:1px solid rgba(48,54,61,0.5);cursor:pointer;" onmouseenter="this.style.background='rgba(88,166,255,0.05)'" onmouseleave="this.style.background=''">
        <div style="flex:1;font-size:14px;color:var(--text);font-weight:500;">Problem #${s.problem_id}: ${escapeHTML(pTitle)}</div>
        <div style="width:100px;font-family:var(--mono);font-size:13px;color:var(--muted);">${s.language === 'python' ? 'Python' : s.language === 'cpp' ? 'C++' : s.language === 'java' ? 'Java' : escapeHTML(s.language)}</div>
        <div style="width:180px;">
          <span class="badge ${verdictClass(s.verdict || s.status)}" style="padding:4px 10px; border-radius: 12px; font-weight: 500;">${escapeHTML(formatVerdict(s.verdict || s.status))}</span>
        </div>
